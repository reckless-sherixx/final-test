            ...
            <hr/>
            {
                ...
                posts.length > 0 ? (
                    <div className='space-y-4 mt-5'>
                        {
                            posts.map((post) => (
                                <Link to={`/Posts/${post._id}`} key={post._id} className='flex flex-col sm:flex-row sm:items-center gap-4 shadow-sm px-8 py-4'>
                                    <div className='h-16 w-16 flex-shrink-0'>
                                        <img src={post.coverImg} alt={post.title} className='h-full w-full rounded-full ring-1 ring-blue-700 transition-transform duration-300 hover:scale-110' />

                                    </div>
                                    <div>
                                        <h4 className='font-medium text-[#1E73BE]'>{post?.title.substring(0, 50)}</h4>
